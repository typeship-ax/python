    ...
    def generate(
        ...
        *,
        ...
        return self._core.request(
            ...
            schema_key="packages.generate",
        )

    def download(self, *, token: str, request_options: Optional[RequestOptions] = None) -> bytes:
        """Download a generated package

        Download the complete ZIP referenced by `packages_generate`'s `download.url`. Pass the
        token from that URL. No API key is needed; the token grants access only to that exact
        package until its replay window expires. Keep the token private.
