...
| --- | --- | --- | --- | --- |
| `idempotency_key` | header | `str` | no | Identifies one logical write for 24 hours. The key is scoped to the authenticated organization and operation; generation without an organization uses a hashed network identity. Retrying the same method, path, query, If-Match header, and JSON body replays the original response. Reusing the key with changed intent returns 409. After expiry the key starts a new write. |
...
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
...
| --- | --- | --- | --- | --- |
| `project_id` | path | `ProjectId` | yes | Accepts an ID or an exact name (resolved via projects_list). IDs come from projects_list. |
| `if_match` | header | `str` | no | ETag from a preceding response. The write applies only if the resource still has that version; otherwise it returns 412 precondition_failed without changes. Omit to write the current version. See https://typeship.dev/docs/typeship-api#conditional-writes. |
...
### `client.spec_revisions.list(*, limit=None, cursor=None, spec_id=None)`
...
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
| `spec_id` | query | `SpecId` | no | Only revisions of this Spec. |

Returns: `Iterator[SpecRevisionRead]` — auto-paginating (`for item in ...` walks every page)
...
### `client.spec_revisions.list_files(spec_revision_id, *, limit=None, cursor=None)`
...
Returns: `Iterator[SpecRevisionFileRead]` — auto-paginating (`for item in ...` walks every page)
Errors: `BadRequestError` (400), `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
### `client.targets.list(*, limit=None, cursor=None, project_id=None)`
...
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
| `project_id` | query | `ProjectId` | no | Only Targets in this Project. |
...
| Parameter | In | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `target_id` | path | `TargetId` | yes | Accepts an ID or an exact name (resolved via targets_list). IDs come from targets_list. |
...
### `client.targets.update(target_id, *, body, if_match=None)`
...
### `client.drafts.list(*, limit=None, cursor=None, target_id=None, status=None)`
...
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
| `target_id` | query | `TargetId` | no | Only Drafts of this Target. |
| `status` | query | `DraftStatus` | no | Only Drafts with this status. |

Returns: `Iterator[DraftRead]` — auto-paginating (`for item in ...` walks every page)
Errors: `BadRequestError` (400), `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
| Parameter | In | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `draft_id` | path | `DraftId` | yes | — |

Returns: `DraftResponseRead`
Errors: `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
| --- | --- | --- | --- | --- |
| `draft_id` | path | `DraftId` | yes | — |
| `filter` | query | `Literal["conflicted", "customized", "history"]` | no | conflicted: conflicts only. customized: files that differ from the last merged package. history: files affected by a default-branch history rewrite. Omit for conflicted and customized files. |
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
...
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
| `target_id` | query | `TargetId` | no | Only releases of this Target. |

Returns: `Iterator[ReleaseRead]` — auto-paginating (`for item in ...` walks every page)
Errors: `BadRequestError` (400), `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
### `client.releases.republish(release_id, *, idempotency_key=None)`
...
### `client.deliveries.list(*, limit=None, cursor=None, target_id=None)`
...
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
| `target_id` | query | `TargetId` | no | Only Deliveries of this Target. |

Returns: `Iterator[DeliveryRead]` — auto-paginating (`for item in ...` walks every page)
Errors: `BadRequestError` (400), `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
### `client.deliveries.get(delivery_id)`
...
### `client.publications.list(*, limit=None, cursor=None, release_id=None)`
...
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
| `release_id` | query | `ReleaseId` | no | Only publications of this release. |

Returns: `Iterator[PublicationRead]` — auto-paginating (`for item in ...` walks every page)
Errors: `BadRequestError` (400), `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
| `limit` | query | `int` | no | Maximum number of resources to return. Omit for 20; otherwise supply base-10 digits representing an integer from 1 to 100. Empty, malformed, or out-of-range values return 400 invalid_request. List query parameters must appear only once; unrecognized parameters also return 400. |
| `cursor` | query | `str` | no | Opaque cursor from the preceding page's next_cursor. Valid only for the same organization, operation, filters, and ordering that issued it. Omit to start at the first page. Empty, malformed, or repeated cursors return 400 invalid_request. The page limit may change between requests. |
| `project_id` | query | `ProjectId` | no | Only Generations in this Project. |
...
| Parameter | In | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `generation_id` | path | `GenerationId` | yes | Accepts an ID or an exact name (resolved via generations_list). IDs come from generations_list. |
...
### `client.generations.list_files(generation_id, *, limit=None, cursor=None)`
...
Returns: `Iterator[FileRead]` — auto-paginating (`for item in ...` walks every page)
Errors: `BadRequestError` (400), `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
| Parameter | In | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `file_id` | path | `FileId` | yes | — |
| `cursor` | query | `str` | no | next_cursor from the preceding chunk of this file. |

Returns: `FileResponseRead`
Errors: `BadRequestError` (400), `UnauthorizedError` (401), `ForbiddenError` (403), `NotFoundError` (404), `RateLimitedError` (429), `InternalServerError` (500)
...
### `client.api_keys.revoke(api_key_id, *, if_match=None)`
